...
public class HTMLCrawler {
	...
	public static Webpage crawl(Webpage web) throws URISyntaxException, IOException {
		...
		for (Element e : meta) {
			if (e.hasAttr("content") && e.attr("content").contains("charset=")) {
				String s = e.attr("content").substring(e.attr("content").indexOf("charset=") + 8);
		...
		web.getWebpageMeta().setLanguage(TextUtils.detectLanguage(doc.text()));
		web.getWebpageMeta().parseMeta(doc);
	 ...
	 */
	private static void saveCache(String targetURL, Webpage web) throws CacheException {
