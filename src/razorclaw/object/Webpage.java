...
public class Webpage implements Serializable, nsICharsetDetectionObserver {
	...
	public void parse() throws IOException {
		...
		} else if (lang.equals("da") || lang.equals("de") || lang.equals("en") || lang.equals("nl")
			...
			for (String p : OpenNLPTokenizer.tokenize(getText(), lang)) {
				...
				if (p != null && !p.isEmpty()) {
					...
					if (getPhraseMap().containsKey(p)) {
						...
						property.setNew(true).setForwardURL(getAPIMeta().getForwardURL()).increaseTFContent();
						getPhraseMap().put(p, property);
					}
				}
			}
		} else if (lang.equals("zh-cn") || lang.equals("zh-tw") || lang.equals("ja") || lang.equals("ka")
				|| lang.equals("vi")) {
			LOG.info("Using CJKV tokenizer");

			CJKVTokenizer.feed(new StringReader(getText()));

			for (String p = CJKVTokenizer.next(); p != null; p = CJKVTokenizer.next()) {
				// skip URL
				if (TextUtils.isURL(p)) {
					continue;
				}
	...
	private void parseProperties() {
		...
		for (String s : getWebpageMeta().getTitle()) {
			...
			if (getPhraseMap().containsKey(s)) {
				...
				property = new PhraseProperty();
				property.increaseTFTitle().setNew(true).setForwardURL(getAPIMeta().getForwardURL());
		...
		for (String s : getWebpageMeta().getKeywords()) {
			...
			if (getPhraseMap().containsKey(s)) {
				...
				property = new PhraseProperty();
				property.increaseTFMetaKeywords().setNew(true).setForwardURL(getAPIMeta().getForwardURL());
		...
		for (String s : getWebpageMeta().getDescription()) {
			...
			if (getPhraseMap().containsKey(s)) {
				...
				property = new PhraseProperty();
				property.increaseTFMetaDescription().setNew(true).setForwardURL(getAPIMeta().getForwardURL());
		...
		for (RefererAnchorText text : getAPIMeta().getRefererAnchorTexts()) {
			...
			if (getPhraseMap().containsKey(s)) {
				...
				property = new PhraseProperty();
				property.increaseTFAnchor().setNew(true).setForwardURL(getAPIMeta().getForwardURL());
		...
		for (String s : getAPIMeta().getSpiderKeywords()) {
			...
			if (getPhraseMap().containsKey(s)) {
				...
				property = new PhraseProperty();
				property.setSpiderKeywords(true).setNew(true).setForwardURL(getAPIMeta().getForwardURL());
		...
		for (String s : getAPIMeta().getAdminKeywords()) {
			...
			if (!s.isEmpty()) {
				if (getPhraseMap().containsKey(s)) {
					...
					property = new PhraseProperty();
					property.setAdminKeywords(true).setNew(true).setForwardURL(getAPIMeta().getForwardURL());
		...
		for (String s : getAPIMeta().getUserKeywords()) {
			...
			if (!s.isEmpty()) {
				if (getPhraseMap().containsKey(s)) {
					...
					property = new PhraseProperty();
					property.setUserKeywords(true).setNew(true).setForwardURL(getAPIMeta().getForwardURL());
		...
		for (Entry<String, PhraseProperty> e : getPhraseMap().entrySet()) {
			e.getValue().setTFScore((double) e.getValue().getTFContent() / getPhraseMap().size());
