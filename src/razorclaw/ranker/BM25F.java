...
public class BM25F {
	...
	private static final double _avgTitleLength = 3.5, _avgMetaKeywordsLength = 5.5, _avgMetaDescriptionLength = 10.5,
			_avgH1Length = 5.5, _avgH2Length = 10.5, _avgContentLength = 123.5, _avgAnchorLength = 4.5,
			_avgUserKeywordsLength = 3.0, _avgAdminKeywordsLength = 3.0, _avgSpiderKeywordsLength = 3.0,
			_avgSearchQueryLength = 2.5;

	// length of phrases(number of characters)
	private static final double _avgLength = 5.5;

	private static final double _titleW = 2.0, _metaKeywordsW = 1.0, _metaDescriptionW = 1.0, _anchorW = 2.0,
			_userKeywordsW = 0.0, _adminKeywordsW = 0.0, _spiderKeywordsW = 0.0, _h1W = 1.5, _h2W = 1.3,
			_contentW = 1.0, _searchQueryW = 1.0, _POSW = 2.0;
	...
	private static final double _contentB = 0.4, _titleB = 0.4, _metaKeywordsB = 0.4, _metaDescriptionB = 0.4,
			_anchorB = 0.4, _adminKeywordsB = 0.4, _spiderKeywordsB = 0.4, _userKeywordsB = 0.4, _h1B = 0.4,
			_h2B = 0.4, _lengthB = 0.4, _searchQueryB = 0.4, _POSB = 0.4;
	...
	public static ArrayList<KeyPhraseScore> rank(
		...
		for (Entry<String, PhraseProperty> e : phrases.entrySet()) {
			...
			// --------------title-------------------
			double titleScore = e.getValue().getTFTitle() * _titleW
					/ ((1 - _titleB) + _titleB * webpageMeta.getTitle().size() / _avgTitleLength);
			kps.setTitleScore(titleScore);
			// ---------------meta keywords---------------
			double metaKeywordsScore = e.getValue().getTFMetaKeywords()
					* _metaKeywordsW
					/ ((1 - _metaKeywordsB) + _metaKeywordsB * webpageMeta.getKeywords().size()
			...
			double metaDescriptionScore = e.getValue().getTFMetaDescription()
					* _metaDescriptionW
					/ ((1 - _metaDescriptionB) + _metaDescriptionB * webpageMeta.getDescription().size()
							/ _avgMetaDescriptionLength);
			kps.setMetaDescriptionScore(metaDescriptionScore);
			// ----------------------h1---------------------
			double h1Score = e.getValue().getTFH1() * _h1W
					/ ((1 - _h1B) + _h1B * webpageMeta.getH1().size() / _avgH1Length);
			kps.setH1Score(h1Score);
			// ----------------------h2------------------
			double h2Score = e.getValue().getTFH2() * _h2W
					/ ((1 - _h2B) + _h2B * webpageMeta.getH2().size() / _avgH2Length);
			kps.setH2Score(h2Score);
			// --------------------content-------------------
			double contentScore = e.getValue().getTFContent() * _contentW
					/ ((1 - _contentB) + _contentB * phrases.size() / _avgContentLength);
			kps.setContentScore(contentScore);
			// ------------anchor text----------------
			double anchorTextScore = e.getValue().getTFAnchor() * _anchorW
					/ ((1 - _anchorB) + _anchorB * apiMeta.getRefererAnchorTexts().size() / _avgAnchorLength);
			...
			// -------------part of speech-----------
			double POSScore = 0.0;
			if (PartOfSpeech.isSignificant(e.getValue().getPartOfSpeech())) {
				POSScore = _POSW;
			}
			...
			// pseudo score
			double pseudoScore = titleScore + metaKeywordsScore + metaDescriptionScore + h1Score + h2Score
					+ contentScore + anchorTextScore + spiderKeywordsScore + adminKeywordsScore + userKeywordsScore
